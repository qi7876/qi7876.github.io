...
const logPrefix = '[LQIP]'
const warnPrefix = '[LQIP][WARN]'
const errorPrefix = '[LQIP][ERROR]'
...
async function generateLqipValue(imagePath: string): Promise<string | null> {
  try {
    ...
    console.error(`${warnPrefix} Failed to process image: ${imagePath}`, error)
    return null
  }
}
...
async function processNewImages(fileMappings: FileMapping[], stats: ImageStats, cleanedMap: LqipMap): Promise<LqipMap> {
  ...
  const processFile = async ({ filePath, webUrl }: FileMapping) => {
    ...
    if (processed % 10 === 0 || processed === stats.new) {
      console.log(`${logPrefix} Processing: ${processed}/${stats.new}`)
    }
  }
  ...
  console.log(`${logPrefix} Generated LQIP styles for ${stats.new} new images`)

  const isNewFile = Object.keys(cleanedMap).length === 0
  await fs.writeFile(lqipMapPath, `${JSON.stringify(newMap, null, 2)}\n`)
  console.log(`${logPrefix} LQIP mapping ${isNewFile ? 'saved to' : 'updated in'} ${lqipMapPath}`)
...
async function applyLqipToHtml(lqipMap: LqipMap): Promise<number> {
  ...
  for (const htmlFile of htmlFiles) {
    try {
      ...
      console.warn(`${warnPrefix} Failed to process ${htmlFile}:`, error)
      continue
    }
  }
 ...
 */
async function main() {
  console.log(`${logPrefix} Starting LQIP processing...`)

  const { fileMappings, imageStats, existingMap } = await scanAndAnalyzeImages()

  if (imageStats.total === 0) {
    console.log(`${logPrefix} No images found to process`)
    return
  }

  console.log(`${logPrefix} Found ${imageStats.total} images (${imageStats.cached} cached, ${imageStats.new} new)`)
  ...
  if (appliedCount === 0) {
    console.log(`${logPrefix} All images already have LQIP styles`)
    return
  }

  console.log(`${logPrefix} Successfully applied LQIP styles to ${appliedCount} images`)
}

main().catch((error) => {
  console.error(`${errorPrefix} LQIP processing failed:`, error)
  process.exit(1)
})
